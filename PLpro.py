...
def generate3DTissue(Nx,Ny,noisx,noisy,direc,dl,dh,nl,cyl,cylCoverage,makePlotHere,datadir):
    ...
    from generateCellMembraneH import generateCellMembrane
    ...
    v,cells,nbs,Lx,Ly = generateCellsPeriodic(Nx,Ny,noisx,noisy,direc=direc,scalingx=0.8, scalingy=1)
    ...
    for ci in range(len(xcells)):
        ...
        scList += [sc];
        ncList += [nc];
        writeVTK_cell(sc,datadir+'cellsPLpro/cell'+str(ci)+'.vtk');
        writeVTK_cell(nc,datadir+'cellsPLpro/nucleus'+str(ci)+'.vtk');
        if makePlotHere:
            for tri in sc.conn:
...
#    set_axes_equal(ax)
    ...
    writeVTK_tissue(scList,datadir+'tissue_memPLpro.vtk')
    writeVTK_tissue(ncList,datadir+'tissue_nucPLpro.vtk')
    return scList
...
if __name__ == "__main__":
    ...
    makePlotHere = 0
    Nx = 4 # width and
    Ny = 8 # length of the tissue in the epithelial plane
    noisx = 0.6
    noisy = 0.6
    direc = 1
    dl = 0.1 # length discretization in plane
    dh = 0.05 # height discretzation 
    nl = 6 # number of height elements
    cylindrical=True
    cylCoverage = 1.0
    datadir = 'cellsGeom/'
